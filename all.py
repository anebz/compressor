...
def constructHuffmanTree(text, progress):
    ...
    for ii in numbers:
        ...
        for key, value in count.items():
            if value == min(dictValues) or value == sorted(dictValues, key=float)[
                1]:  # if value is smallest or second-smallest value
                flag += 1
                if flag == 1:
                    node1 = key
                    for jj in key:
...
def generalcompression(origin_data):
    # got the foldername, allinfo from all the txt files under main dir
    ...
    fold = ''
    fold2 = ''
    foldercompression(f)  # all files in main dir
    while (1):
        for (dirpath2, dirnames, filenames) in os.walk(dirpath + '/' + fold):  # now dirs
            fold2 = dirnames
            # for foldername in fold2:


def foldercompression(f):
...
def compression(progress):
    ...
    if foldername != '':
        generalcompression(origin_data)
        return
...
def open_origin_dir():
    global origin_path, foldername, files, dirpath, dirs
